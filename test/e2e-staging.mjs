...
const defaultDesktopRoot = '/path/to/beeper/desktop'
...
const otp = process.env.BEEPER_E2E_OTP || '[REDACTED_OTP]'
...
async function main() {
  ...
  const profiles = Array.from({ length: accountCount }, (_, index) => ({
    ...
    email: process.env[`BEEPER_E2E_EMAIL_${index + 1}`] || `staging-user+${emailBase + index}@beeper.com`,
  }))
...
async function loginInstance(instance) {
  ...
  if (status?.state === 'needs-login') {
    const username = instance.email.match(/\+(\d+)@/)?.[1] ? `staging-user${RegExp.$1}` : `staging-user${runID}${instance.index}`
...
async function runAuthenticatedReadCommands(instance, context) {
  ...
  await runCli(['contacts', 'search', 'staging-user', '--account', context.accountID, '--json'], { instance, allowFailure: true })
