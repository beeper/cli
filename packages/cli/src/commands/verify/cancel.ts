...
export default class VerifyCancel extends BeeperCommand {
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(VerifyCancel)
    ensureWritable(flags)
    const id = flags.id
    const path = `/v1/app/setup/verifications/:id/cancel`.replace(':id', encodeURIComponent(id ?? 'active'))
