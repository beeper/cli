...
description: Call raw Desktop API endpoints the CLI doesn't yet wrap with a workflow command.
sidebar:
...
<Aside type="tip" title="Read when">
  Calling raw Desktop API endpoints that the CLI doesn't yet wrap with a
  workflow command — so scripts can reach a new endpoint before a dedicated
  command exists.
</Aside>
...
- `api request` lets you hit `GET | POST | PUT | PATCH | DELETE`; the others are
  convenience shortcuts.
- `--read-only` blocks `api post` / `api put` / `api patch` / `api delete` /
  `api request <write>`.
...
The full Desktop API surface is documented at the
